...
public class SSEKnowledgeBaseBuilder {

    final static Log LOG = LogFactory.getLog(SSEKnowledgeBaseBuilder.class);
    ...
    public static void main(String[] args) throws IOException, ConfigurationException, CompressorException {
        ...
        SSEKnowledgeBaseBuilder kbb = new SSEKnowledgeBaseBuilder();

        Properties config = new Properties();
        String confFile = args[0];
        config.load(new FileInputStream(new File(confFile)));
        String sseKBPath = config.getProperty("sse.kb", "").trim();
        ...
        Directory directory = FSDirectory.open(new File(sseKBPath));
